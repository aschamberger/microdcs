...
- **Keeping `docs/operations.md` in sync**: whenever a field is added to or removed from `RedisConfig`, `MQTTConfig`, `MessagePackConfig`, or `ProcessingConfig` in `src/microdcs/__init__.py`, update the corresponding `APP_*` table in `docs/operations.md` — including the variable name, type, default value, and description. The four config sections map directly to the four table subsections: `APP_REDIS_*`, `APP_MQTT_*`, `APP_MSGPACK_*`, `APP_PROCESSING_*`.
...
- **SFC action completion routing is instance-affine by design**: `_pending_commands` (push_command response routing) and `_pull_event_keys` (pull_event routing) are in-memory maps on the dispatching instance. If MQTT delivers a response/event to a different live instance, the lookup misses and the event is silently dropped — the action stays stuck in Redis until pod restart triggers `_recovery_scan`. For push_command when the instance **dies** before ACKing the stream entry, XAUTOCLAIM recovers automatically. This is intentional: moving routing tables to Redis would add a round-trip per incoming event. See `docs/sfc_engine.md` "Multi-Instance Safety Summary" for the full matrix.
- **`recovery_idle_threshold_s` parameter exists but is currently unused**: `SfcEngine.__init__` accepts this parameter and stores it, but `_recovery_scan` currently enqueues `resume` for all active incomplete jobs without filtering by idle time. Do not add threshold filtering without also updating the docs.
- **`register_pull_completion_handler` is not wired in `app/__main__.py`**: The example app only uses `push_command` actions, so no pull_event wiring is needed there. Any app using `pull_event` actions must call `sb_processor.register_pull_completion_handler(sfc_engine.complete_pull_action)` during wiring.
